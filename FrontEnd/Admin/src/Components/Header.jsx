...
export default function Header({ onMenuClick, title = "پنل مدیریت هنرستان" }) {
    ...
    <header className="sticky top-0 z-30 h-16 sm:h-20 bg-white/90 dark:bg-[#0B0F17]/90 backdrop-blur-md border-b border-gray-200/80 dark:border-gray-800 px-4 sm:px-6 flex items-center justify-between transition-colors">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center justify-center w-6 h-6 rounded-lg bg-[#202A5A]/10 dark:bg-white/10 text-[#202A5A] dark:text-white">
              <ShieldCheck className="w-3.5 h-3.5 text-[#59BBAF]" />
            </span>
            <span className="text-xs font-bold text-gray-500 dark:text-gray-400 hidden sm:inline-block">
              پنل دبیران و معاونان • هنرستان رُکاد
            </span>
          </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative" ref={userRef}>
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-[#202A5A] text-white flex items-center justify-center font-black text-xs">
              {user?.fullName ? user.fullName[0] : <UserIcon className="w-4 h-4" />}
            </div>
            <div className="text-right hidden sm:block">
              <span className="block text-xs font-bold text-[#202A5A] dark:text-white truncate max-w-[120px]">
                {user?.fullName || 'کادر مدیریت'}
              </span>
              <span className="block text-[10px] text-gray-400">
                {user?.role === 'superAdmin' ? 'مدیر ارشد' : 'معاونت / دبیر'}
              </span>
            </div>
          </button>
            ...
            <div className="absolute left-0 mt-2 w-48 bg-white dark:bg-[#151C28] rounded-2xl border border-gray-200 dark:border-gray-700 shadow-[3px_3px_0_#202A5A] dark:shadow-[3px_3px_0_#59BBAF] p-2 z-50 animate-in fade-in zoom-in-95 duration-150">
              <div className="p-2 border-b border-gray-100 dark:border-gray-800 text-right">
                ...
                <p className="text-[11px] text-gray-400 mt-0.5">
                  کد پرسنلی: {toPersianDigits(user?.idCode || '')}
                </p>
              </div>
              <button
